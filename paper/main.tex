...
We use simulations from the Abacus project \cite{abacus}. 

We use the simulation boxes with size $L_{\rm box}=720$\hMpc simulated
with $1440^3$ particles and a resulting particle mass of $\sim 1\times
10^{10}$\hMpc. 
We use the set of 41 boxes with different $\omega$CDM cosmological
parameters which are phase-matched to isolate cosmology dependences. 


\section{Sample Selection}

We start by selectin all halos with a maximum circular velocity larger
than $200$ \kms.
...
closer than the $AB$ separation; in other words in IPs the two halos
are mutually its closest neighbor.
